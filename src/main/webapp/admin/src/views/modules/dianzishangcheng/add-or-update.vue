<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="商品编号" prop="shangpinbianhao">
          <el-input v-model="ruleForm.shangpinbianhao" 
              placeholder="商品编号" clearable  :readonly="ro.shangpinbianhao"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="商品编号" prop="shangpinbianhao">
              <el-input v-model="ruleForm.shangpinbianhao" 
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'" label="商品演示" prop="shangpinyanshi">
          <file-upload
          tip="点击上传商品演示"
          action="file/upload"
          :limit="1"
          :multiple="true"
          :fileUrls="ruleForm.shangpinyanshi?ruleForm.shangpinyanshi:''"
          @change="shangpinyanshiUploadChange"
          ></file-upload>
        </el-form-item> 
        <div v-else>
          <el-form-item v-if="ruleForm.shangpinyanshi" label="商品演示" prop="shangpinyanshi">
            <el-button type="text" size="small" @click="download(ruleForm.shangpinyanshi)">预览</el-button>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="上市时间" prop="shangshishijian">
            <el-date-picker
                format="yyyy 年 MM 月 dd 日"
                value-format="yyyy-MM-dd"
                v-model="ruleForm.shangshishijian" 
                type="date"
                placeholder="上市时间">
            </el-date-picker> 
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.shangshishijian" label="上市时间" prop="shangshishijian">
              <el-input v-model="ruleForm.shangshishijian" 
                placeholder="上市时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  data() {
	...
	shangpinyanshi : false,
	shangshishijian : false,
	xiangxicanshu : false,
	thumbsupnum : false,
	crazilynum : false,
	price : false,
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.shangpintupian = true;
          ...
          if(o=='shangpinyanshi'){
            this.ruleForm.shangpinyanshi = obj[o];
	    this.ro.shangpinyanshi = true;
            continue;
          }
          if(o=='shangshishijian'){
            this.ruleForm.shangshishijian = obj[o];
	    this.ro.shangshishijian = true;
            continue;
          }
          if(o=='xiangxicanshu'){
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `dianzishangcheng/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.dianzishangchengCrossAddOrUpdateFlag = false;
                  this.parent.search();
    ...
    shangpinyanshiUploadChange(fileUrls) {
	this.ruleForm.shangpinyanshi = fileUrls;
			this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
